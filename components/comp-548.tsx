...
export default function FileUploader() {
  const maxSize = 30 * 1024 * 1024 // 10MB default
  ...
  async function handleFileUpload(file: File) {
    try {
      // 1. Get a temporary upload URL from your backend
      ...
      // 2. Upload the file to the URL using Pinata SDK
      const upload = await pinata.upload.public.file(file).url(url);
  
      // 3. Handle the uploaded result (IPFS hash or accessible URL)
      console.log("Upload result:", upload);
  
      // Optionally update UI or form value
      return upload;
    } catch (error) {
